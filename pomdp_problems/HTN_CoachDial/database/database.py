...
class DB_Object(object):
    ...
    # Find and return the all possible values for a specific attribute of an
    # object
    ...
    def ability_check(self, precond, state):
        for i, x in enumerate(state):
            if i == 0:
                continue
            if float(state[i]) < float(precond[i]):
                return False
        return True
    ...
    # Find and return the prob for p(obs|s) ##@@This is p(obs|sensor), prob of
    # sensor notification| state, dependant on realiability, read from
    # sensor_0.99 json
    ...
    def update_sensor_value(self, ob_name, attri_name, value, real_step=False):
        ...
        if len(sensor) != 1:
            print(
                "inside udpate_sensor_value, the number of target ob_name is bad",
                len(sensor))
            sys.exit(0)

        # in this case the sensor is missing just return False
        elif sensor[0]["reliability"] == -1.0:
    ...
    def update_state_belief_for_exception(
            self, ob_name, attri_name, attri_value):
    ...
    def get_obj_Rstate(self, ob_name):
        ...
        if len(objList) != 1:
            print(
                "inside get_obj_Rstate, the number of target ob_name is bad",
                len(objList))
            sys.exit(0)
        else:
            return objList[0]

    # Update the object real state given the ob_name, attri_name, and
    # attri_value
