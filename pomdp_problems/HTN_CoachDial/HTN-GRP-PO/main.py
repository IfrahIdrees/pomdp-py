...
def get_output_path(args):
    ...
    os.makedirs("../logs/{}/{}/mcts".format(args.agent_type,
                                            output_name), exist_ok=True)
    os.makedirs("../logs/{}/{}/reward".format(args.agent_type,
                                              output_name), exist_ok=True)
...
if __name__ == '__main__':
    ...
    delete_trigger = 0.001
    # config.mcts_delete_trigger = 0.001
    # 0.000095 #(0.000095 works with time)
    config._real_delete_trigger = 0.00001
    ...
    # orignal other_happen = 0.75
    # other_happen = 0.30
    # 0.87 #(use 0.87 for all the cases between except 7 and 9, for 7 and 9
    # use 0.86) #0.85 #0.87
    other_happen = 0.86
    ...
    config.randomNs = [
        random.random() for i in range(
            (21) * 1000 * args.num_sims * 20)]
    ...
    for file_num in file_nums:
        ...
        for x in sensor_reliability:
            ...
            for repeat in range(1, trials):
                ...
                tracking_engine = Tracking_Engine(
                    ...
                    mcts_output_filename=mcts_output_filename,
                    args=args,
                    db_client=db_client)
                ...
                cumulative_reward_df.loc[len(cumulative_reward_df.index)] = ([args.num_sims,
                                                                              total_reward_per_iter,
                                                                              total_discounted_reward_per_iter,
                                                                              num_question_asked_per_iter,
        ...
        # import csv
            with open('config_info.csv', 'a', newline='') as csvfile:
                ...
                # spamwriter.writerow(['Spam'] * 5 + ['Baked Beans'])
                spamwriter.writerow([args.maxsteps, args.num_sims, args.d, args.e, file_num, str(
                    x), str(repeat), config.randomIndex, config.realRandomIndex])
else:
