...
class State(pomdp_py.State):
    def __init__(self, instruction_step, user_intent, user_activity, world_state, time_step):
        """
    ...
    def __repr__(self):
        return "State(%s | %s | %s | %s | %s | %s)" % (str(self.instruction_step), str(self.user_intent), str(self.user_activity), str(self.world_state), str(self.is_terminal), str(self.time_step_entered))
    
    def other_user_intent(self):
        # When we raise the size of the problem, modify this function
        if self.user_intent.name == "saying-nothing":
            return UserAffirmDoingStepIntent()
        else:
            return UserSayingNothingIntent()
    
    def other_user_activity(self):
        # When we raise the size of the problem, modify this function
        if self.user_activity.name == "doing-nothing":
            return UserPuttingSaucepanOnStoveActivity()
        else:
            return UserDoingNothingActivity()
...
class CDTransitionModel(pomdp_py.TransitionModel):
    ...
    def _read_simulation_table(self):
        ...
        table = pd.read_csv(args.simulation_table_path, dtype={"w": str, "w'": str})
        return table
    ...
    def _change_world(self, prev_state, prev_agent_action):
        ...
        if any(condition_all):
            ...
            logger.info("TIMESTEP IS %s", str(TIME_STEP))
            return State(curr_i, curr_u, curr_actv, curr_w, TIME_STEP)
        else:
            # otherwise, return previous state
            #logger.info("I am returning the previous state")
            return State(prev_state.instruction_step, prev_state.user_intent, prev_state.user_activity, prev_state.world_state, prev_state.time_step_entered)
    ...
    def sample(self, prev_state, prev_action):
        ...
        #logger.info("prev_is_terminal %s", str(prev_is_terminal))
        ## condition to check if it is the end. 
        if prev_is_terminal:
            # Already terminated, so no state transition happens.
            prev_is_terminal = True
            logger.info("In transition function current state is %s", prev_state.__str__())
            return State(prev_state.instruction_step, prev_state.user_intent, prev_state.user_activity, prev_state.world_state, prev_state.time_step_entered)
        else:
            ...
            #logger.info("after change world %s", str(prev_is_terminal))
            if(prev_action.name == "bye"):
                prev_is_terminal = True
                next_state.is_terminal = True
            
            logger.info("In transition function current state is %s", next_state.__str__())
            return next_state 
...
#        print("cur_state", curr_state, "\naction", action)
        ...
        if random.uniform(0,1) < thresh:
            #print("In observation model", curr_state.user_intent, curr_state.world_state, curr_state.user_activity)
            logger.info("In observation model curr_state.user_intent %s | curr_state.world_state %s | curr_state.user_activity %s", curr_state.user_intent, curr_state.world_state, curr_state.user_activity)
            return Observation(curr_state.user_intent, curr_state.world_state, curr_state.user_activity)
        else:
            return Observation(curr_state.user_intent, curr_state.world_state, curr_state.other_user_activity())
...
#        self.is_terminal = False
    ...
    def sample(self, state, action, next_state, normalized=False, **kwargs):
        ...
        if isinstance(action, AgentGiveNextInstructionAction):
            ...
            logger.info("MAX_INSTRUCTION_STEP == 1 %s, and next_state.instruction_step > MAX_INSTRUCTION_STEP %s ", MAX_INSTRUCTION_STEP == 1, next_state.instruction_step > MAX_INSTRUCTION_STEP)
            logger.info("state.instruction_step != MAX_INSTRUCTION_STEP %s, activity same %s", state.instruction_step != MAX_INSTRUCTION_STEP,state.user_activity == "doing-nothing")
            if state.instruction_step in update_i_step_condition_dict.keys() :
                logger.info("world state same %s", update_i_step_condition_dict[state.instruction_step]['w'] == state.world_state)
        #     if MAX_INSTRUCTION_STEP == 1:
        #         if next_state.instruction_step > MAX_INSTRUCTION_STEP:
        #             return args.give_next_instr_penalty
        #         elif(update_i_step_condition_dict[state.instruction_step]['w'] == state.world_state and  state.user_activity == "doing-nothing"):
        ...
        #     else:     
        #    # if (state.instruction_step > MAX_INSTRUCTION_STEP):
        #     #     return args.give_next_instr_penalty
        ##@II 5/24/21 remove max_instruction condition MAX_INSTRUCTION_STEP == 1 and
            if next_state.instruction_step > MAX_INSTRUCTION_STEP:
                return args.give_next_instr_penalty
            if (
                # state.instruction_step != MAX_INSTRUCTION_STEP and 
...
class CoachDialProblem(pomdp_py.POMDP):
    ...
    def generate_instance(n, k):
        ...
        init_state = State(instruction_step, user_intent, user_activity, world_state, TIME_STEP) 
...
def test_planner(coachdial, planner, nsteps=3, discount=0.95):
    ...
    simulation_table = read_simulation_table()
    optimal_count = 0
    ...
    for i in range(nsteps):
        ...
        if IS_RANDOM_AGENT:
            ...
            coachdial.agent.update_history(action, real_observation)
            planner.update(coachdial.agent, action, real_observation) #pomcp.update
        ...
        # Automatic evaluation
        optimal_actions = get_optimal_action(true_state, simulation_table)
        if str(action) in optimal_actions:
            optimal_count += 1

        # Printing states
        print("True prev state: %s" % true_state)
        print("Action: %s" % str(action))
        print("Observation: %s" % str(real_observation))
        print("True curr state: %s" % true_next_state)
        ...
        # pomdp_py.visual.visualize_pouct_search_tree(coachdial.agent.tree,max_depth=5, anonymize=False)
...
def read_simulation_table():
    """Read simulation table and return a pd.DataFrame"""
    table = pd.read_csv(args.simulation_table_path, dtype={"w": str, "w'": str})
    return table

def get_optimal_action(curr_state, simulation_table):
    i_step = curr_state.instruction_step
    u = curr_state.user_intent.name
    w = curr_state.world_state
    actv = curr_state.user_activity.name

    condition_i = simulation_table.i==i_step
    condition_u = simulation_table.u==u
    condition_w = simulation_table.w==w
    condition_actv = simulation_table.actv==actv
       
    condition_all = condition_i & condition_u & condition_w & condition_actv

    if any(condition_all):
        candidate_states = simulation_table[condition_all]
        optimal_actions = set(candidate_states["a"].to_list())
...
def main(args):
    ...
    for run_id in range(args.num_runs):
        ...
        pomcp = pomdp_py.POMCP(max_depth=8, discount_factor=0.95,
                               num_sims=args.num_sims, exploration_const=50,
                               rollout_policy=coachdial.agent.policy_model,
                               num_visits_init=1)
        outputs = test_planner(coachdial, pomcp, nsteps=args.nsteps, discount=0.95)
        # pomdp_py.visual.visualize_pouct_search_tree(coachdial.agent.tree,max_depth=6, anonymize=False)
        ...
        optimal_count, total_steps = outputs["optimal_count"], outputs["total_steps"]
        logger.info(f"Optimal actions: {optimal_count} / {total_steps}")
        logger.info("DONE!")
        ...
        nsteps_list.append(outputs["total_steps"])
        optimal_count_list.append(outputs["optimal_count"])
    ...
    optimal_count_list = np.array(optimal_count_list)
    optimal_coverage = np.mean(optimal_count_list/nsteps_list)

    logger.info(f"{args.num_runs:4d} runs") 
    logger.info(f"nsteps_list: {nsteps_list}")
    logger.info(f"optimal_count_list: {optimal_count_list}")
    logger.info(f"")
    ...
    logger.info(f"Optimal action coverage: {optimal_coverage:8.6f}")
