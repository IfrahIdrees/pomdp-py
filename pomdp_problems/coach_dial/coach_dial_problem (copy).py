...
class CDTransitionModel(pomdp_py.TransitionModel):
    ...
    def __init__(self, k, transition_constant):
       """Read in the transition table and return the corresponding world state in the simulation table 
        ...
        self.simulation_table = _read_simulation_table()

    def _read_simulation_table(self):
        """Read simulation table and return a pd.DataFrame"""
        table = pd.read_csv('simulation_table.csv')
        return table
    ...
    def _change_world(self, prev_state, prev_agent_action):
        ...
        prev_i = state.instruction_step.name
        prev_u = state.user_intent.name
        prev_w = state.world_state
        prev_actv = state.user_activity.name
        prev_a = prev_agent_action.name
        
        # lookup conditions
        condition_i = self.simulation_table.i==prev_i_step
        condition_u = self.simulation_table.u==prev_u
        condition_w = self.simulation_table.w==prev_world_state
        ...
        if any(condition_all):
            ...
            sampled_state_id = np.random.choice(num_candidate_states, 1)
            sampled_state = candidate_states.iloc[sampled_state_id]

            curr_i = sampled_state["i'"]

            curr_u_name = sampled_state["u'"]
            curr_u = Action(curr_u_name) #TODO: Change to specific action, e.g. UserDoingNothingActivity

            curr_w = sampled_state["w'"]

            curr_actv_name = sampled_state["actv'"]
            curr_actv = Action(curr_actv_name)
    ...
    def sample(self, prev_state, prev_action):
        ...
        if prev_is_terminal:
            ...
            next_state = _change_world(prev_state, prev_action)
...
class CDRewardModel(pomdp_py.RewardModel):
    def __init__(self, terminating_effect_dict):
#        self._appliance_locs = appliance_locs
        self._last_instruction_step = False
        self._instruction_terminating_effect = terminating_effect_dict
	#terminating_effect_dict ={instruction_step: {end_world_state}}
	#self.instruction_step = instruction_step
#        self.user_intent = user_intent
#        self.user_activity = user_activity
#        self.world_state = world_state
#        self.is_terminal = False
    ...
    def sample(self, state, action, next_state, normalized=False, **kwargs):
        ...
        if isinstance(action, AgentGiveNextInstructionAction):
            if terminating_effect_dict[state.instruction_step]['w'] == next_state.world_state && next_state.user_activity == "doing_nothing":
                return 10
            else:
                return -10
...
class CoachDialProblem(pomdp_py.POMDP):
    ...
    def generate_instance(n, k):
        ...
        instruction_step = 0
        user_intent = UserSayingNothingAction()
    ...
    def __init__(self, n, k, init_state, appliance_locs, init_belief, transition_constant, terminating_effect_dict):
        self._n, self._k = n, k
        agent = pomdp_py.Agent(init_belief,
                               CDPolicyModel(n,k),
                               CDTransitionModel(k, transition_constant),
                               CDObservationModel(),
                               CDRewardModel(appliance_locs, self.in_exit_area))
        env = pomdp_py.Environment(init_state,
                                   CDTransitionModel(k, transition_constant),
                                   CDRewardModel(appliance_locs, self.in_exit_area))
        self._appliance_locs = appliance_locs
        self.terminating_effect_dict = terminating_effect_dict
...
if __name__ == '__main__':
    ...
    terminating_effect_dict ={1:{'w':'0001'}, 2:{'w':'0011'}, 3:{'w':'1011'}, 4:{'w':'1111'}, 5:{'w':'0111'}}
    belief = "uniform"
    ...
    coachdial = CoachDialProblem(n, k, init_state, appliance_locs=None, init_belief, transition_constant, terminating_effect_dict)
