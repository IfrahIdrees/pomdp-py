...
#        self.is_terminal = False
    ...
    def sample(self, state, action, next_state, normalized=False, **kwargs):
        ...
        if isinstance(action, AgentGiveNextInstructionAction):
            ...
            logger.info("state.instruction_step != MAX_INSTRUCTION_STEP %s, world state same %s, activity same %s", state.instruction_step != MAX_INSTRUCTION_STEP, update_i_step_condition_dict[state.instruction_step]['w'] == state.world_state, state.user_activity == "doing-nothing")
            # if (state.instruction_step > MAX_INSTRUCTION_STEP):
            #     return args.give_next_instr_penalty
            if (update_i_step_condition_dict[state.instruction_step]['w'] == state.world_state and 
